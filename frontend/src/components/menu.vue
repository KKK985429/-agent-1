...
const topMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) =>
        item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'organizations' || item.path === 'creatChat' || item.path === 'medical/departments' || item.path === 'medical/search'
    );
});

const bottomMenuItems = computed<MenuItem[]>(() => {
    return (visibleMenuArr.value as unknown as MenuItem[]).filter((item: MenuItem) => {
        if (item.path === 'knowledge-bases' || item.path === 'agents' || item.path === 'organizations' || item.path === 'creatChat' || item.path === 'medical/departments' || item.path === 'medical/search') {
            return false;
        }
        return true;
    });
...
const getIcon = (path: string) => {
    ...
    knowledgeIcon.value = (kbActiveState.isKbActive || route.name === 'medicalDepartments' || route.name === 'medicalSearch') ? 'zhishiku-green.svg' : 'zhishiku.svg';
