export default {
  ...
  newUserGuide: {
    ...
    steps: {
      welcome: {
        title: "欢迎使用 AIP-wisdom",
        desc: "只需几步，带你快速了解知识库、对话与智能体的核心用法。点击「下一步」开始。",
      },
      knowledge: {
        title: "创建你的知识库",
        desc: "知识库是一切的起点：上传文档、网页或 FAQ，AIP-wisdom 会自动解析并建立索引。点击这里进入知识库。",
      },
      chat: {
        title: "发起智能对话",
  ...
  settings: {
    ...
    roleDenied: {
      ...
      description: "配置 AIP-wisdom Cloud 的 APPID 和 APPSECRET 凭证。凭证用于模型服务和文档解析引擎。",
      viewDocs: "查看文档",
      unconfigured: "尚未配置凭证，请填写 APPID 和 APPSECRET",
      configured: "凭证已配置，状态正常",
      expired: "AIP-wisdom Cloud 凭证已失效",
      expiredDefault: "服务重启后加密密钥已变更，已保存的凭证无法解密。请重新填写凭证。",
      reconfigure: "重新配置",
      appIdLabel: "APPID",
      appIdDesc: "AIP-wisdom Cloud 的应用 ID",
      appIdPlaceholder: "请输入 APPID",
      appSecretLabel: "APPSECRET",
      appSecretDesc: "AIP-wisdom Cloud 的应用密钥",
      appSecretPlaceholder: "请输入 APPSECRET",
      saveHint: "保存后将验证服务可达性并加密存储凭证",
      saveBtn: "保存凭证",
      usageTitle: "使用说明",
      usageSteps: "1. 填写并保存 APPID / APPSECRET\n2. 在下方「云模型接入」中按行添加 chat、embedding、rerank、vlm\n3. 文档解析：知识库设置 → 解析引擎，选择 AIP-wisdom Cloud 引擎",
      fillRequired: "请填写 APPID 和 APPSECRET",
      saveSuccess: "凭证保存成功",
      saveFailed: "凭证保存失败",
      credentialConfigured: "AIP-wisdom Cloud 凭证已配置",
      credentialExpired: "凭证已失效，请重新配置。",
      credentialUnconfigured: "尚未配置 AIP-wisdom Cloud 凭证，请先填写 APPID 和 APPSECRET。",
      checkingStatus: "正在检查凭证状态...",
      goToSettings: "前往设置中配置",
      modelHintConfigured: "AIP-wisdom Cloud 凭证已配置。支持的模型可参考",
      modelHintDocsLink: "接口文档",
      modelsSection: {
        title: "云模型接入",
        descReady: "将 AIP-wisdom Cloud 提供的四类标准模型注册到当前空间，用于对话、向量检索、重排序与多模态理解。",
  ...
  createChat: {
    title: "Hi，我是 AIP-wisdom，让你的知识触手可及",
    newSessionTitle: "新会话",
    messages: {
    ...
    parser: {
      ...
      engines: {
        ...
        weknoracloud: {
          ...
          desc: "使用 AIP-wisdom Cloud 进行文档解析",
        },
        markitdown: {
          name: "MarkItDown",
